...
async function _getFallbackDb() {
  let local = localStorage.getItem('statecraft_live_db');
  if (local) {
    try { return JSON.parse(local); } catch (e) {}
  }
  try {
    const res = await fetch('data/default_states.json');
    if (res.ok) {
      const data = await res.json();
      localStorage.setItem('statecraft_live_db', JSON.stringify(data));
      return data;
    }
  } catch (e) {}
  return { states: [], teams: [], adminConfig: { adminPasscode: 'akshita' }, buzzer: { enabled: false, showResults: false, round: "Round 1", buzzes: [] }, round2: { enabled: false, title: "Round 2", teamSelections: {} } };
}

function _saveFallbackDb(data) {
  try {
    localStorage.setItem('statecraft_live_db', JSON.stringify(data));
  } catch (e) {}
}
...
const StateCraftAPI = {
  ...
  async adminLogin(passcode) {
    try {
      ...
      if (res.status === 401) return { success: false, error: "Invalid Admin Passcode" };
    } catch (e) {
      console.warn('Admin login API offline, using fallback auth:', e);
    }
    const db = await _getFallbackDb();
    const correctCode = db.adminConfig?.adminPasscode || 'akshita';
    if (String(passcode).trim() === correctCode) {
      return {
        success: true,
        message: "Admin authenticated successfully",
        adminToken: "adm_" + Math.random().toString(36).substring(2)
      };
    }
  ...
  async participantLogin(teamName, password) {
    try {
      ...
      if (res.ok) return await res.json();
      if (res.status === 401) return { success: false, error: "Invalid Team Name or Password" };
    } catch (e) {
      console.warn('Participant login API offline, using fallback auth:', e);
    }
    const db = await _getFallbackDb();
    const cleanName = String(teamName).trim().toLowerCase();
    const team = (db.teams || []).find(t => t.name.trim().toLowerCase() === cleanName && t.password === String(password).trim());
    if (!team) {
      return { success: false, error: "Invalid Team Name or Password" };
    }
    const state = team.allocatedStateId ? (db.states || []).find(s => s.id === team.allocatedStateId) : null;
    return { success: true, team, allocatedState: state };
  },
  ...
  async createTeam(teamData) {
    ...
    const newTeam = {
      ...
      allocatedStateId: teamData.allocatedStateId || null,
      points: 0,
      developmentScore: 0,
    ...
    _saveFallbackDb(db);
    broadcastStateChange('TEAM_CREATED', newTeam);
  ...
  async updateTeam(teamId, teamData) {
    ...
    if (team) {
      if (teamData.name) team.name = teamData.name;
      if (teamData.password) team.password = teamData.password;
      if (teamData.allocatedStateId !== undefined) team.allocatedStateId = teamData.allocatedStateId || null;
      _saveFallbackDb(db);
      broadcastStateChange('TEAM_UPDATED', team);
  ...
  async awardPoints(teamId, pointsChange, category, reason) {
    ...
    const team = (db.teams || []).find(t => t.id === teamId);
    if (team) {
      team.points = Math.max(0, (team.points || 0) + Number(pointsChange));
      team.pointLogs = team.pointLogs || [];
      const logEntry = {
      ...
      _saveFallbackDb(db);
      broadcastStateChange('POINTS_AWARDED', { teamId, pointsChange, team });
      return { success: true, team, addedLog: logEntry };
    }
    return { success: false, error: "Team not found" };
  },
  ...
  async updateStateCriteria(stateId, criteriaUpdates) {
    ...
    if (state && criteriaUpdates.criteria) {
      Object.assign(state.criteria, criteriaUpdates.criteria);
      _saveFallbackDb(db);
      broadcastStateChange('STATE_CRITERIA_UPDATED', state);
  ...
  async resetDatabase() {
    try {
      const res = await fetch(`${STATECRAFT_API_BASE}/api/reset`, { method: 'POST' });
      if (res.ok) {
        const result = await res.json();
    ...
    localStorage.removeItem('statecraft_live_db');
    broadcastStateChange('DATABASE_RESET', {});
    return { success: true, message: "Database reset to initial defaults" };
  },
  ...
  async fetchBuzzerState() {
    ...
    const db = await _getFallbackDb();
    return db.buzzer || { enabled: false, showResults: false, round: "Round 1", buzzes: [] };
  },
  ...
  async toggleBuzzer(enabled) {
    ...
    const db = await _getFallbackDb();
    db.buzzer = db.buzzer || { enabled: false, showResults: false, round: "Round 1", buzzes: [] };
    db.buzzer.enabled = Boolean(enabled);
    db.buzzer.activatedAt = enabled ? new Date().toISOString() : null;
    _saveFallbackDb(db);
    broadcastStateChange('BUZZER_TOGGLED', db.buzzer);
    return { success: true, buzzer: db.buzzer };
  },
  ...
  async revealBuzzerOrder(showResults) {
    ...
    const db = await _getFallbackDb();
    db.buzzer = db.buzzer || {};
    db.buzzer.showResults = Boolean(showResults);
    _saveFallbackDb(db);
    broadcastStateChange('BUZZER_REVEALED', db.buzzer);
    return { success: true, buzzer: db.buzzer };
  },
  ...
  async resetBuzzer(roundName) {
    ...
    const db = await _getFallbackDb();
    db.buzzer = { enabled: false, showResults: false, round: roundName || "Round 1", activatedAt: null, buzzes: [] };
    _saveFallbackDb(db);
    broadcastStateChange('BUZZER_RESET', db.buzzer);
    return { success: true, buzzer: db.buzzer };
  },
  ...
  async submitBuzz(teamId) {
    ...
    const db = await _getFallbackDb();
    db.buzzer = db.buzzer || { enabled: false, buzzes: [] };
    const team = (db.teams || []).find(t => t.id === teamId) || { name: 'Team', id: teamId };
    ...
    db.buzzer.buzzes = db.buzzer.buzzes || [];
    db.buzzer.buzzes.push(buzzRecord);
    _saveFallbackDb(db);
    broadcastStateChange('BUZZER_PRESSED', buzzRecord);
    return { success: true, buzz: buzzRecord, order: orderNum, buzzer: db.buzzer };
  },
  ...
  async fetchRound2State() {
    ...
    const db = await _getFallbackDb();
    return db.round2 || { enabled: false, title: "Round 2", teamSelections: {} };
  },
  ...
  async toggleRound2(enabled) {
    ...
    const db = await _getFallbackDb();
    db.round2 = db.round2 || {};
    db.round2.enabled = Boolean(enabled);
    _saveFallbackDb(db);
    broadcastStateChange('ROUND2_TOGGLED', db.round2);
    return { success: true, round2: db.round2 };
  },
  ...
  async resetRound2(teamId = null, disable = false) {
    ...
    const db = await _getFallbackDb();
    db.round2 = db.round2 || {};
    if (teamId && db.round2.teamSelections) {
      delete db.round2.teamSelections[teamId];
    } else {
      db.round2.teamSelections = {};
      if (disable) db.round2.enabled = false;
    }
    _saveFallbackDb(db);
    broadcastStateChange('ROUND2_RESET', db.round2);
    return { success: true, round2: db.round2 };
  },
  ...
  async submitRound2Choice(payload) {
    ...
    const db = await _getFallbackDb();
    db.round2 = db.round2 || {};
    db.round2.teamSelections = db.round2.teamSelections || {};
    db.round2.teamSelections[payload.teamId] = payload;
    _saveFallbackDb(db);
    broadcastStateChange('ROUND2_SUBMITTED', payload);
    return { success: true, selection: payload, round2: db.round2 };
  },
  ...
  async fetchMarketConfig() {
    ...
    try {
      const res = await fetch('data/market_config.json');
      if (res.ok) {
  ...
  async buyInfrastructure(teamId, password, itemId) {
    try {
      ...
      if (res.ok) {
        const result = await res.json();
        if (result.success) broadcastStateChange('INFRASTRUCTURE_PURCHASED', { teamId, purchase: result.purchase, team: result.team });
        return result;
      }
    } catch (e) {}
    return { success: true, message: "Infrastructure added" };
  },
